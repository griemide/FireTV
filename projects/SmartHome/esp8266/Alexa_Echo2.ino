...
char packetBuffer[UDP_TX_PACKET_MAX_SIZE]; //buffer to hold incoming packet, // #define UDP_TX_PACKET_MAX_SIZE 24 in WiFiUdp.h
//char packetBuffer[28]; //buffer to hold incoming packet

String serial;
String persistent_uuid;


const int relayPin = 04;
...
void setup() {
  ...
  if(wifiConnected){
    ...
    if (udpConnected){
      ...
      startHttpServer();
      Serial.println("User interaction required: Ask Alexa to discover devices");
    }
  }  
}
...
boolean connectWifi(){
  ...
  if (state){
    ...
    Serial.println("");
    Serial.println("Connection to Wifi ssid failed.");
  }
...
boolean connectUDP(){
  ...
  if(UDP.beginMulticast(WiFi.localIP(), ipMulti, portMulti)) {
    Serial.println("Connection to UDP successful");
    state = true;
  }
  else{
